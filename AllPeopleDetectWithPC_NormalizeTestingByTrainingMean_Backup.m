...
[ Train, Test, DataComponent] = DivideTrainTestData( DataList, 0.5, 1 );
...
%% PNN
NormalTraining = cell2mat(Train.Normal(:,3));
NormalTraining = NormalTraining(:,Mask);

AbnormalTraining = cell2mat(Train.Abnormal(:,3));
AbnormalTraining = AbnormalTraining(:,Mask);

clear Train

TrainingSet = [NormalTraining', AbnormalTraining'];
...
TrainingTarget = [zeros(1, size(NormalTraining,1) ) ones(1, size(AbnormalTraining, 1) ) ] +1;
...
for pcaIteration = 1:size(Latent,1)
 ...
 TestNormal = cell2mat(Test.Normal(:,3));
 TestNormal = TestNormal(:,Mask);
 
 TestAbnormal = cell2mat(Test.Abnormal(:,3));
 TestAbnormal = TestAbnormal(:,Mask);
 
 clear Test
 
Test_NormalScore = EigVec'*(TestNormal'-repmat(mean_Training, 1, size(TestNormal',2) ) );
Test_AbnormalScore = EigVec'*(TestAbnormal'-repmat(mean_Training, 1, size(TestAbnormal',2) ) );
