...
for n = 1:length(InputDomain)
    ...
    if ~isempty(NormalPerIdx)
        if normal_testing_num ~= 0
            Test.Normal = [Test.Normal NormalSet(Mask, 1:normal_testing_num)];
            Test.NormalPer = [Test.NormalPer; NormalPerIdx(1:normal_testing_num) ];
        end
        Train.Normal = [Train.Normal NormalSet(Mask, normal_testing_num+1:end)];
        Train.NormalPer = [Train.NormalPer; NormalPerIdx(normal_testing_num+1:end) ];
    end
    
    if ~isempty(AbnormalPerIdx)
        if abnormal_testing_num ~= 0
            Test.Abnormal = [Test.Abnormal AbnormalSet(Mask, 1:abnormal_testing_num)];
            Test.AbnormalPer = [Test.AbnormalPer; AbnormalPerIdx(1:abnormal_testing_num) ];
        end
        Train.Abnormal = [Train.Abnormal AbnormalSet(Mask, abnormal_testing_num+1:end)];
        Train.AbnormalPer = [Train.AbnormalPer; AbnormalPerIdx(1:abnormal_testing_num+1:end) ];
    end
...
sensitivity = [];
specificity = [];
for pcaIteration = 1:1%size(Latent,1)
    PCAComNum = pcaIteration;
    Trained_PNN = Train_PNN(Score(:,1:PCAComNum)', TrainingTarget, PNN_Spares);

 %% Confusion Matrix
 
Test_NormalScore = EigVec'*( Test.Normal ./ repmat(std_Training, 1, size(Test.Normal,2) ) );
Test_AbnormalScore = EigVec'*( Test.Abnormal  ./ repmat(std_Training, 1, size(Test.Abnormal,2) ) );
