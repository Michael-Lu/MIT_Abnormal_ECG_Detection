...
%% PNN
TrainingSet = [zscore(Train.Normal')' zscore(Train.Abnormal')'];

% apply PCA to TrainingSet
mean_Normal_Training = mean(Train.Normal,2);
std_Normal_Training = std(Train.Normal,0,2);

mean_Abnormal_Training = mean(Train.Abnormal,2);
std_Abnormal_Training = std(Train.Abnormal,0,2);
...
Test_NormalScore = EigVec'* ( (Test.Normal-repmat(mean_Normal_Training, 1, size(Test.Normal,2)) )./ repmat(std_Normal_Training, 1, size(Test.Normal,2)) );
Test_AbnormalScore = EigVec'* ( (Test.Abnormal-repmat(mean_Abnormal_Training, 1, size(Test.Abnormal,2)) ) ./ repmat(std_Abnormal_Training, 1, size(Test.Abnormal,2)) );
